...
use super::{MelParams, MelStats, GlobalStats};
...
impl MelOps {
    ...
    pub fn normalize_min_max(mel: &mut MelSpectrogram) -> Result<()> {
        ...
        if range == 0.0 {
            return Err(AcousticError::InputError("Cannot normalize constant signal".to_string()));
        }
    ...
    pub fn normalize_robust_scale(mel: &mut MelSpectrogram) -> Result<()> {
        ...
        if iqr == 0.0 {
            return Err(AcousticError::InputError("Cannot normalize with zero IQR".to_string()));
        }
    ...
    pub fn normalize_unit_norm(mel: &mut MelSpectrogram) -> Result<()> {
        ...
        if norm == 0.0 {
            return Err(AcousticError::InputError("Cannot normalize zero signal".to_string()));
        }
    ...
    fn compute_median_iqr(values: &[f32]) -> Result<(f32, f32)> {
        if values.is_empty() {
            return Err(AcousticError::InputError("Empty input for median/IQR computation".to_string()));
        }
    ...
    pub fn time_stretch(mel: &MelSpectrogram, factor: f32) -> Result<MelSpectrogram> {
        if factor <= 0.0 {
            return Err(AcousticError::InputError("Time stretch factor must be > 0".to_string()));
        }
        ...
        for (mel_idx, channel) in mel.data.iter().enumerate() {
            for new_frame_idx in 0..new_n_frames {
                let original_frame = (new_frame_idx as f32 * factor) as f32;
        ...
        Ok(MelSpectrogram::new(new_data, mel.sample_rate, mel.hop_length))
    }
        ...
        for frame_idx in 0..mel.n_frames {
            for (mel_idx, new_channel) in new_data.iter_mut().enumerate() {
                ...
                if mel_low < mel.data.len() && mel_high < mel.data.len() {
                    new_channel[frame_idx] = 
                        mel.data[mel_low][frame_idx] * (1.0 - frac) + 
    ...
    pub fn concatenate(mels: &[&MelSpectrogram]) -> Result<MelSpectrogram> {
        if mels.is_empty() {
            return Err(AcousticError::InputError("Cannot concatenate empty list".to_string()));
        }
        ...
        for mel in mels.iter().skip(1) {
            if mel.n_mels != n_mels {
                return Err(AcousticError::InputError("All mel spectrograms must have same n_mels".to_string()));
            }
            if mel.sample_rate != sample_rate {
                return Err(AcousticError::InputError("All mel spectrograms must have same sample_rate".to_string()));
            }
            if mel.hop_length != hop_length {
                return Err(AcousticError::InputError("All mel spectrograms must have same hop_length".to_string()));
            }
        }
        ...
        Ok(MelSpectrogram::new(concatenated_data, sample_rate, hop_length))
    }
    
    /// Extract a slice from mel spectrogram
    pub fn slice(mel: &MelSpectrogram, start_frame: usize, end_frame: usize) -> Result<MelSpectrogram> {
        if start_frame >= end_frame {
            return Err(AcousticError::InputError("Start frame must be < end frame".to_string()));
        }
        if end_frame > mel.n_frames {
            return Err(AcousticError::InputError("End frame exceeds mel spectrogram length".to_string()));
        }
        ...
        Ok(MelSpectrogram::new(sliced_data, mel.sample_rate, mel.hop_length))
    }
    ...
    pub fn pad(
        ...
        Ok(MelSpectrogram::new(padded_data, mel.sample_rate, mel.hop_length))
    }
    
    /// Apply smoothing filter to mel spectrogram
    pub fn smooth(mel: &mut MelSpectrogram, kernel_size: usize) -> Result<()> {
        if kernel_size == 0 || kernel_size % 2 == 0 {
            return Err(AcousticError::InputError("Kernel size must be odd and > 0".to_string()));
        }
    ...
    pub fn compute_delta(mel: &MelSpectrogram, window_size: usize) -> Result<MelSpectrogram> {
        if window_size == 0 {
            return Err(AcousticError::InputError("Window size must be > 0".to_string()));
        }
        ...
        Ok(MelSpectrogram::new(delta_data, mel.sample_rate, mel.hop_length))
    }
    ...
    pub fn resize(
        ...
        if target_n_mels == 0 || target_n_frames == 0 {
            return Err(AcousticError::InputError("Target dimensions must be > 0".to_string()));
        }
        ...
        for new_mel_idx in 0..target_n_mels {
            for new_frame_idx in 0..target_n_frames {
                ...
                let mel_coord = (new_mel_idx as f32) * (mel.n_mels as f32 - 1.0) / (target_n_mels as f32 - 1.0);
                let frame_coord = (new_frame_idx as f32) * (mel.n_frames as f32 - 1.0) / (target_n_frames as f32 - 1.0);
                ...
                if mel_low < mel.data.len() && mel_high < mel.data.len() &&
                   frame_low < mel.data[mel_low].len() && frame_high < mel.data[mel_low].len() &&
                   frame_low < mel.data[mel_high].len() && frame_high < mel.data[mel_high].len() {
                    ...
                    resized_data[new_mel_idx][new_frame_idx] = v1 * (1.0 - mel_frac) + v2 * mel_frac;
                }
            }
        }
        
        Ok(MelSpectrogram::new(resized_data, mel.sample_rate, mel.hop_length))
    }
}
...
mod tests {
    ...
    fn test_normalize_z_score() {
        ...
        let global_mean: f32 = mel.data.iter().flatten().sum::<f32>() / 
            (mel.n_mels * mel.n_frames) as f32;
