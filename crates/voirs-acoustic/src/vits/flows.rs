...
impl InvertibleConv1x1 {
    pub fn new(n_channels: usize, device: &Device) -> CandleResult<Self> {
        ...
        for i in 0..weight_data.len() {
            weight_data[i] += (fastrand::f32() - 0.5) * 0.1;
        }
    ...
    pub fn forward(&self, x: &Tensor) -> CandleResult<(Tensor, Tensor)> {
        ...
        let x_reshaped = x.permute((0, 2, 1))?.reshape((batch_size * n_frames, n_channels))?;
        ...
        let y = y_reshaped.reshape((batch_size, n_frames, n_channels))?.permute((0, 2, 1))?;
    ...
    pub fn inverse(&self, y: &Tensor) -> CandleResult<(Tensor, Tensor)> {
        ...
        let y_reshaped = y.permute((0, 2, 1))?.reshape((batch_size * n_frames, n_channels))?;
        
        // Apply inverse transformation
        let x_reshaped = y_reshaped.matmul(&weight_inv.t()?)?;
        
        // Reshape back
        let x = x_reshaped.reshape((batch_size, n_frames, n_channels))?.permute((0, 2, 1))?;
        
        // Negative log determinant
        let log_det = (self.log_det_weight.neg()?.broadcast_as((batch_size,))? * (n_frames as f64))?;
...
impl WaveNet {
    ...
    pub fn forward(&self, x: &Tensor) -> CandleResult<Tensor> {
        ...
        // Apply to each frame
        let x_reshaped = x.permute((0, 2, 1))?.reshape((batch_size * n_frames, in_channels))?;
        let output_reshaped = x_reshaped.matmul(&weight)?;
        let output = output_reshaped.reshape((batch_size, n_frames, in_channels))?.permute((0, 2, 1))?;
...
impl NormalizingFlows {
    ...
    pub fn load_with_varbuilder(
        ...
        for i in 0..config.n_flows {
            ...
            ).map_err(|e| AcousticError::ModelError(format!("Failed to create flow step {}: {}", i, e)))?;
    ...
    pub fn forward(&mut self, z: &Tensor) -> Result<(Tensor, Tensor)> {
        ...
        let mut total_log_det = Tensor::zeros((z.dims()[0],), DType::F32, &self.device)
            .map_err(|e| AcousticError::ModelError(format!("Failed to create log_det tensor: {}", e)))?;
        
        tracing::debug!("NormalizingFlows forward: input shape {:?}", z.dims());
        
        for (i, step) in self.flow_steps.iter_mut().enumerate() {
            let (new_z, log_det) = step.forward(&current_z)
                .map_err(|e| AcousticError::ModelError(format!("Flow step {} forward failed: {}", i, e)))?;
            
            current_z = new_z;
            total_log_det = (&total_log_det + log_det)
                .map_err(|e| AcousticError::ModelError(format!("Log det accumulation failed at step {}: {}", i, e)))?;
    ...
    pub fn inverse(&self, z: &Tensor) -> Result<(Tensor, Tensor)> {
        ...
        for (i, step) in self.flow_steps.iter().enumerate().rev() {
            let (new_z, log_det) = step.inverse(&current_z)
                .map_err(|e| AcousticError::ModelError(format!("Flow step {} inverse failed: {}", i, e)))?;
            
            current_z = new_z;
            total_log_det = (&total_log_det + log_det)
                .map_err(|e| AcousticError::ModelError(format!("Log det accumulation failed at step {}: {}", i, e)))?;
            
            tracing::debug!("Flow step {} inverse: output shape {:?}", i, current_z.dims());
        }
