...
impl DurationPredictor {
    ...
    pub fn load_with_varbuilder(
        ...
        for i in 0..config.n_layers {
            ...
            ).map_err(|e| AcousticError::ModelError(format!("Failed to create conv block {}: {}", i, e)))?;
    ...
    pub fn forward(&self, text_encoding: &Tensor) -> Result<Tensor> {
        ...
        if input_shape.len() != 3 {
            return Err(AcousticError::InputError(
                format!("Expected 3D tensor [batch, input_dim, seq_len], got {:?}", input_shape)
            ));
        }
        
        let (batch_size, input_dim, seq_len) = text_encoding.dims3()
            .map_err(|e| AcousticError::ModelError(format!("Failed to get tensor dimensions: {}", e)))?;
        
        if input_dim != self.config.input_dim {
            return Err(AcousticError::InputError(
                format!("Expected {} input dimensions, got {}", self.config.input_dim, input_dim)
            ));
        }
        ...
        // Input convolution
        let mut h = self.input_conv.forward(text_encoding)
        ...
        for (i, block) in self.conv_blocks.iter().enumerate() {
            h = block.forward(&h)
                .map_err(|e| AcousticError::ModelError(format!("Conv block {} failed: {}", i, e)))?;
        }
        
        tracing::debug!("After conv blocks: {:?}", h.dims());
        
        // Output convolution to predict log-durations
        let log_durations = self.output_conv.forward(&h)
    ...
    pub fn predict_durations(&self, text_encoding: &Tensor, inference: bool) -> Result<Tensor> {
        ...
        let mut durations = log_durations.exp()
        ...
        durations = durations.maximum(&ones)
    ...
    pub fn predict_phoneme_durations(&self, phonemes: &[Phoneme]) -> Result<Vec<f32>> {
        ...
        for phoneme in phonemes {
            let base_duration = match phoneme.symbol.as_str() {
                ...
                "AA" | "AE" | "AH" | "AO" | "AW" | "AY" | "EH" | "ER" | "EY" | "IH" | "IY" | "OW" | "OY" | "UH" | "UW" => 8.0,
                // Consonants
                "B" | "CH" | "D" | "DH" | "F" | "G" | "HH" | "JH" | "K" | "L" | "M" | "N" | "NG" | "P" | "R" | "S" | "SH" | "T" | "TH" | "V" | "W" | "Y" | "Z" | "ZH" => 4.0,
                // Special tokens
                "<pad>" | "<unk>" => 1.0,
                "<bos>" | "<eos>" => 2.0,
                // Default
            ...
            let variation = (fastrand::f32() - 0.5) * 0.4 + 1.0; // 0.8 to 1.2
    ...
    pub fn align_text_to_mel(&self, text_encoding: &Tensor, durations: &Tensor) -> Result<Tensor> {
        let (batch_size, input_dim, seq_len) = text_encoding.dims3()
            .map_err(|e| AcousticError::ModelError(format!("Failed to get text encoding dimensions: {}", e)))?;
        
        let (dur_batch, dur_channels, dur_seq) = durations.dims3()
            .map_err(|e| AcousticError::ModelError(format!("Failed to get duration dimensions: {}", e)))?;
        
        // Validate dimensions
        if batch_size != dur_batch || seq_len != dur_seq || dur_channels != 1 {
            return Err(AcousticError::InputError(
                format!("Dimension mismatch: text [{}, {}, {}], durations [{}, {}, {}]", 
                       batch_size, input_dim, seq_len, dur_batch, dur_channels, dur_seq)
            ));
        }
        
        // Squeeze duration channel dimension
        let durations = durations.squeeze(1)
        ...
        let total_frames = durations.sum_all()
            .map_err(|e| AcousticError::ModelError(format!("Failed to sum durations: {}", e)))?
            .to_scalar::<f32>()
            .map_err(|e| AcousticError::ModelError(format!("Failed to convert to scalar: {}", e)))? as usize;
        ...
        let aligned = text_encoding.repeat(&[1, 1, repeat_factor])
            .map_err(|e| AcousticError::ModelError(format!("Failed to align text encoding: {}", e)))?;
...
pub fn duration_based_upsampling(
    ...
    let (batch_size, channels, seq_len) = text_encoding.dims3()
    ...
    for batch_idx in 0..batch_size {
        ...
        for seq_idx in 0..seq_len {
            let duration = durations
                ...
                .get(seq_idx)?
                .to_scalar::<f32>()
                .map_err(|e| AcousticError::ModelError(format!("Failed to get duration scalar: {}", e)))? as usize;
            
            let frame_encoding = text_encoding
                .get(batch_idx)?
                .narrow(1, seq_idx, 1)?; // [channels, 1]
        ...
        if !batch_output.is_empty() {
            let batch_tensor = Tensor::cat(&batch_output.iter().collect::<Vec<_>>(), 1)
                .map_err(|e| AcousticError::ModelError(format!("Failed to concatenate frames: {}", e)))?;
            output_data.push(batch_tensor);
        }
    }
    
    if output_data.is_empty() {
        return Err(AcousticError::ModelError("No output data generated".to_string()));
    }
