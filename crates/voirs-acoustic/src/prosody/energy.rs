...
impl EnergyConfig {
    ...
    pub fn calculate_energy(&self, phoneme: &Phoneme, context: &EnergyContext) -> f32 {
        ...
        energy *= self.contour_pattern.get_energy_factor(context.position_in_phrase);
    ...
    pub fn validate(&self) -> Result<()> {
        if !(0.0..=1.0).contains(&self.base_energy) {
            return Err(AcousticError::ConfigError(
                format!("Base energy {} must be between 0.0 and 1.0", self.base_energy)
            ));
        }
        
        if self.dynamic_range < 1.0 || self.dynamic_range > 60.0 {
            return Err(AcousticError::ConfigError(
                format!("Dynamic range {} dB is out of range (1-60 dB)", self.dynamic_range)
            ));
        }
        
        if !(0.0..=1.0).contains(&self.breathiness) {
            return Err(AcousticError::ConfigError(
                format!("Breathiness {} must be between 0.0 and 1.0", self.breathiness)
            ));
        }
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        for (name, value) in &fields {
            if !(0.0..=1.0).contains(value) {
                return Err(AcousticError::ConfigError(
                    format!("{} {} must be between 0.0 and 1.0", name, value)
                ));
            }
        }
        ...
        if self.formant_shift < -12.0 || self.formant_shift > 12.0 {
            return Err(AcousticError::ConfigError(
                format!("Formant shift {} semitones is out of range (-12 to 12)", self.formant_shift)
            ));
        }
