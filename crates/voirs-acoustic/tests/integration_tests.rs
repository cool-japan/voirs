...
use voirs_acoustic::{DummyAcousticModel, AcousticModel, Phoneme, SynthesisConfig, MelSpectrogram};
...
fn convert_acoustic_mel_to_vocoder(acoustic_mel: voirs_acoustic::MelSpectrogram) -> voirs_vocoder::MelSpectrogram {
...
fn convert_acoustic_config_to_vocoder(acoustic_config: &voirs_acoustic::SynthesisConfig) -> voirs_vocoder::SynthesisConfig {
...
async fn test_end_to_end_pipeline() {
    ...
    println!("Generated {} phonemes: {:?}", phonemes.len(), 
             phonemes.iter().map(|p| &p.symbol).collect::<Vec<_>>());
    
    // Step 2: Phonemes to mel spectrogram (Acoustic Model)
    let phonemes_converted: Vec<voirs_acoustic::Phoneme> = phonemes.iter().map(|p| convert_g2p_phoneme_to_acoustic(p)).collect();
    let config = SynthesisConfig::default();
    let mel = acoustic.synthesize(&phonemes_converted, Some(&config)).await.unwrap();
    assert!(mel.n_mels > 0, "Acoustic model should produce mel spectrogram");
    assert!(mel.n_frames > 0, "Mel spectrogram should have frames");
    println!("Generated mel spectrogram: {} mels x {} frames", mel.n_mels, mel.n_frames);
    ...
    let audio = vocoder.vocode(&mel_converted, Some(&vocoder_config)).await.unwrap();
    assert!(audio.duration() > 0.0, "Vocoder should produce audio with duration");
    assert!(!audio.is_empty(), "Audio should not be empty");
    println!("Generated audio: {:.2}s at {}Hz", audio.duration(), audio.sample_rate());
    
    // Verify the complete pipeline works
    let total_duration = audio.duration();
    assert!(total_duration > 0.1, "Audio should be at least 100ms for 'Hello, world!'");
    ...
    println!("   Mel: {}x{} spectrogram", mel_n_mels, mel_n_frames);
...
async fn test_batch_synthesis() {
    ...
    let converted_phonemes: Vec<Vec<voirs_acoustic::Phoneme>> = all_phonemes.iter()
        .map(|phonemes| phonemes.iter().map(|p| convert_g2p_phoneme_to_acoustic(p)).collect())
        .collect();
    
    // Batch acoustic synthesis
    let phoneme_refs: Vec<&[voirs_acoustic::Phoneme]> = converted_phonemes.iter().map(|p| p.as_slice()).collect();
    let mels = acoustic.synthesize_batch(&phoneme_refs, None).await.unwrap();
    assert_eq!(mels.len(), test_texts.len(), "Should generate mel for each input");
    
    // Batch vocoding
    let mels_converted: Vec<voirs_vocoder::MelSpectrogram> = mels.into_iter().map(|m| convert_acoustic_mel_to_vocoder(m)).collect();
    let audios = vocoder.vocode_batch(&mels_converted, None).await.unwrap();
    assert_eq!(audios.len(), test_texts.len(), "Should generate audio for each mel");
    
    // Verify all audios have reasonable properties
    for (i, audio) in audios.iter().enumerate() {
        assert!(audio.duration() > 0.0, "Audio {} should have duration", i);
...
async fn test_synthesis_config_effects() {
    ...
    for (i, config) in configs.iter().enumerate() {
        ...
        assert!(audio.duration() > 0.0, "Config {} should produce valid audio", i);
        println!("Config {}: {:.2}s audio (speed: {}, pitch: {})", 
                 i, audio.duration(), config.speed, config.pitch_shift);
    }
