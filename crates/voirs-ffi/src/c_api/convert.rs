...
use crate::{VoirsErrorCode, VoirsAudioBuffer};
...
pub unsafe extern "C" fn voirs_convert_sample_rate(
    ...
    if input.is_null() || output.is_null() || output_samples.is_null() ||
       input_samples == 0 || input_rate == 0 || output_rate == 0 || channels == 0 {
        return VoirsErrorCode::InvalidParameter;
    }

    let input_slice = std::slice::from_raw_parts(input, input_samples as usize);
    let ratio = output_rate as f64 / input_rate as f64;
    let output_length = ((input_samples as f64 / channels as f64) * ratio) as usize * channels as usize;
...
pub unsafe extern "C" fn voirs_audio_convert_format(
    ...
    // For now, we only support float32 input format
    // TODO: Implement other input formats when needed
    
    // Convert sample rate if needed
    if target_sample_rate != audio_buffer.sample_rate {
        let mut new_samples: *mut c_float = std::ptr::null_mut();
        ...
        let max_output_samples = ((audio_buffer.length as f64 / audio_buffer.channels as f64) * 
                                 (target_sample_rate as f64 / audio_buffer.sample_rate as f64)) as usize * 
        ...
        if result != VoirsErrorCode::Success {
            std::alloc::dealloc(temp_buffer as *mut u8, 
                std::alloc::Layout::from_size_align(
                    max_output_samples * std::mem::size_of::<f32>(),
                    std::mem::align_of::<f32>(),
                ).unwrap());
