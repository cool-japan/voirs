...
impl LjSpeechDataset {
    /// Load LJSpeech dataset from directory
    pub async fn load<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        if !base_path.exists() {
            return Err(DatasetError::LoadError(format!(
                "LJSpeech dataset not found at {:?}. Use download() method first.",
                base_path
            )));
        }
        ...
        if !metadata_path.exists() {
            return Err(DatasetError::LoadError(
                "metadata.csv not found in LJSpeech dataset".to_string()
            ));
        }
        ...
        for (_index, result) in csv_reader.records().enumerate() {
            let record = result?;
            if record.len() < 3 {
                continue;
            }
            ...
            let audio_path = base_path.join("wavs").join(format!("{}.wav", file_id));
            ...
            // Load audio data
            let audio_data = match load_audio_file(&audio_path).await {
                Ok(audio) => audio,
                Err(e) => {
            ...
            let sample = DatasetSample {
                ...
                speaker: Some(SpeakerInfo {
                    id: "linda_johnson".to_string(),
                    name: Some("Linda Johnson".to_string()),
                    gender: Some("female".to_string()),
                    age: None,
                    accent: Some("North American".to_string()),
                ...
                metadata: {
                    ...
                    meta.insert("original_text".to_string(), serde_json::Value::String(original_text));
                    meta.insert("file_id".to_string(), serde_json::Value::String(file_id));
                    meta.insert("audio_path".to_string(), serde_json::Value::String(audio_path.to_string_lossy().to_string()));
                    meta
                },
            };
    ...
    pub async fn download<P: AsRef<Path>>(download_path: P) -> Result<()> {
        ...
        archive.unpack(download_path)
            .map_err(|e| DatasetError::IoError(e))?;
        
        tracing::info!("Successfully downloaded and extracted LJSpeech dataset to {:?}", download_path);
        Ok(())
    }
    ...
    fn create_duration_splits(&self, config: SplitConfig) -> Result<DatasetSplits> {
        ...
        let mut indexed_samples: Vec<(usize, f32)> = self.samples.iter()
            .enumerate()
    ...
    fn create_text_length_splits(&self, config: SplitConfig) -> Result<DatasetSplits> {
        ...
        let mut indexed_samples: Vec<(usize, usize)> = self.samples.iter()
            .enumerate()
    ...
    fn split_by_indices(&self, indices: Vec<usize>, config: SplitConfig) -> Result<DatasetSplits> {
        ...
        if val_size == 0 && config.val_ratio > 0.0 {
            return Err(DatasetError::SplitError("Validation split would be empty".to_string()));
        }
        if test_size == 0 && config.test_ratio > 0.0 {
            return Err(DatasetError::SplitError("Test split would be empty".to_string()));
        }
        ...
        let train_samples = train_indices.iter()
            .map(|&i| self.samples[i].clone())
            .collect();
        let val_samples = val_indices.iter()
            .map(|&i| self.samples[i].clone())
            .collect();
        let test_samples = test_indices.iter()
            .map(|&i| self.samples[i].clone())
            .collect();
...
impl Dataset for LjSpeechDataset {
    ...
    async fn validate(&self) -> Result<ValidationReport> {
        ...
        for (i, sample) in self.samples.iter().enumerate() {
            ...
            if sample.text.chars().any(|c| !c.is_ascii() && !c.is_whitespace()) {
                warnings.push(format!("Sample {}: Contains non-ASCII characters", i));
            }
            ...
            if let Some(audio_path) = sample.metadata.get("audio_path") {
                if let Some(path_str) = audio_path.as_str() {
                    ...
                    if !path.exists() {
                        errors.push(format!("Sample {}: Audio file not found: {:?}", i, path));
                    }
                }
            }
        }
    ...
    let samples: Result<Vec<f32>> = match spec.sample_format {
        hound::SampleFormat::Int => {
            match spec.bits_per_sample {
                ...
                _ => Err(DatasetError::AudioError(format!("Unsupported bit depth: {}", spec.bits_per_sample))),
            }
        },
        hound::SampleFormat::Float => {
            let float_samples: Vec<f32> = reader.samples::<f32>()
                .collect::<std::result::Result<Vec<_>, _>>()
                .map_err(|e| DatasetError::AudioError(format!("Failed to read samples: {}", e)))?;
...
mod tests {
    ...
    fn test_duration_balanced_splits() {
        ...
        assert!((train_ratio - 0.8).abs() < 0.2, "Train duration ratio: {}", train_ratio);
        assert!((val_ratio - 0.1).abs() < 0.1, "Val duration ratio: {}", val_ratio);
        assert!((test_ratio - 0.1).abs() < 0.1, "Test duration ratio: {}", test_ratio);
    }
