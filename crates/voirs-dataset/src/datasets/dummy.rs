...
impl DummyDataset {
    ...
    pub fn with_config(config: DummyConfig) -> Self {
        ...
        for i in 0..config.num_samples {
            ...
            let sample = DatasetSample {
                id: format!("dummy_{:06}", i),
                text,
                audio,
                speaker: Some(SpeakerInfo {
                    id: format!("dummy_speaker_{}", i % 3), // 3 different speakers
                    name: Some(format!("Dummy Speaker {}", i % 3)),
                ...
                metadata: {
                    ...
                    meta.insert("audio_type".to_string(), serde_json::Value::String(format!("{:?}", config.audio_type)));
                    meta.insert("text_type".to_string(), serde_json::Value::String(format!("{:?}", config.text_type)));
                    meta.insert("sample_index".to_string(), serde_json::Value::Number(i.into()));
                    meta
                },
            };
        ...
        let metadata = DatasetMetadata {
            ...
            metadata: {
                ...
                meta.insert("audio_type".to_string(), serde_json::Value::String(format!("{:?}", config.audio_type)));
                meta.insert("text_type".to_string(), serde_json::Value::String(format!("{:?}", config.text_type)));
                meta.insert("sample_rate".to_string(), serde_json::Value::Number(config.sample_rate.into()));
                meta.insert("channels".to_string(), serde_json::Value::Number(config.channels.into()));
                meta
            },
        };
    ...
    fn generate_audio(config: &DummyConfig, duration: f32, rng: &mut StdRng) -> AudioData {
        ...
        match config.audio_type {
            AudioType::SineWave => {
                ...
                let angular_frequency = 2.0 * std::f32::consts::PI * frequency / config.sample_rate as f32;
    ...
    fn generate_text(config: &DummyConfig, index: usize, rng: &mut StdRng) -> String {
        match config.text_type {
            ...
            TextType::Numbers => {
                let formats = vec![
                    format!("This is sample number {}", index + 1),
                    format!("Number {} in the sequence", index + 1),
                    format!("Item {} of the dummy dataset", index + 1),
                    format!("Sample {} generated for testing", index + 1),
                    format!("Audio clip number {}", index + 1),
                ];
            ...
            TextType::RandomWords => {
                let words = vec![
                    "hello", "world", "test", "audio", "speech", "synthesis", "voice", "dataset",
                    "sample", "text", "processing", "natural", "language", "machine", "learning",
                    "artificial", "intelligence", "deep", "neural", "network", "model", "training",
                    "evaluation", "performance", "quality", "metrics", "validation", "testing",
                    "development", "research", "science", "technology", "computer", "software",
                    "algorithm", "data", "analysis", "experiment", "result", "conclusion"
                ];
    ...
    async fn validate(&self) -> Result<ValidationReport> {
        ...
        for (i, sample) in self.samples.iter().enumerate() {
            ...
            if sample.audio.sample_rate() == 0 {
                errors.push(format!("Sample {}: Invalid sample rate", i));
            }
            
            // Check for valid channels
            if sample.audio.channels() == 0 {
                errors.push(format!("Sample {}: Invalid channel count", i));
            }
        }
