...
impl Default for RoomConfig {
    fn default() -> Self {
        Self {
            room_types: vec![RoomType::SmallRoom, RoomType::MediumRoom, RoomType::LargeRoom],
            reverb_time: 1.2,
            early_delay: 20.0,
            reverb_level: 0.3,
            damping: 0.5,
...
impl RoomAugmentor {
    ...
    pub fn apply_room_simulation(&self, audio: &AudioData, room_type: RoomType) -> Result<AudioData> {
    ...
    fn apply_impulse_response_convolution(&self, audio: &AudioData, room_type: RoomType) -> Result<AudioData> {
        let samples = audio.samples();
        let sample_rate = audio.sample_rate();
        let channels = audio.channels();
    ...
    fn add_late_reverberation(&self, ir: &mut [f32], start_idx: usize, rt60: f32, damping: f32) -> Result<()> {
...
impl ParametricReverb {
    ...
    fn initialize_filters(&mut self, rt60: f32, early_delay: f32, damping: f32) {
        ...
        for &delay in &allpass_delays {
            self.allpass_filters.push(AllpassFilter::new(delay + early_samples, 0.7));
        }
        ...
        for &delay in &comb_delays {
            self.comb_filters.push(CombFilter::new(delay + early_samples * 2, feedback, damping));
        }
    }
...
impl BatchRoomProcessor {
    ...
    pub fn process_batch(&self, audio_files: &[AudioData]) -> Result<(Vec<Vec<AudioData>>, RoomStats)> {
