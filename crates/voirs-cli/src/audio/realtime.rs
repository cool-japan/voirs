...
use cpal::{
    Device, Host, Stream, SampleRate, ChannelCount, 
...
impl RealTimeAudioStream {
    ...
    pub fn new(
        stream_config: RealTimeStreamConfig, 
        buffer_config: BufferConfig,
    ) -> Result<Self> {
    ...
    pub fn write_audio(&self, audio_data: &AudioData) -> Result<()> {
        let samples_f32: Vec<f32> = audio_data.samples.iter()
    ...
    pub fn write_samples(&self, samples: &[f32]) -> Result<()> {
        let mut write_idx = self.write_index.lock()
            .map_err(|_| VoirsError::device_error("audio_stream", "Failed to lock write_index mutex"))?;
        
        let mut buffers = self.buffers.lock()
            .map_err(|_| VoirsError::device_error("audio_stream", "Failed to lock buffers mutex"))?;
    ...
    pub fn is_active(&self) -> Result<bool> {
        let active = self.is_active.lock()
            .map_err(|_| VoirsError::device_error("audio_stream", "Failed to lock is_active mutex"))?;
        Ok(*active)
    }
    
    /// Set active state
    fn set_active(&self, active: bool) -> Result<()> {
        let mut state = self.is_active.lock()
            .map_err(|_| VoirsError::device_error("audio_stream", "Failed to lock is_active mutex"))?;
        *state = active;
        Ok(())
    }
    ...
    pub fn get_buffer_fill_level(&self) -> Result<f32> {
        let write_idx = self.write_index.lock()
            .map_err(|_| VoirsError::device_error("audio_stream", "Failed to lock write_index mutex"))?;
        let read_idx = self.read_index.lock()
            .map_err(|_| VoirsError::device_error("audio_stream", "Failed to lock read_index mutex"))?;
        
        let buffers = self.buffers.lock()
            .map_err(|_| VoirsError::device_error("audio_stream", "Failed to lock buffers mutex"))?;
        
        let ready_buffers = buffers.iter()
            .filter(|b| b.is_ready)
            .count();
    ...
    pub fn get_estimated_latency_ms(&self) -> Result<f32> {
        ...
        let buffer_duration_ms = (self.buffer_config.buffer_size as f32 / self.config.sample_rate as f32) * 1000.0;
        let total_buffer_latency = buffer_fill * buffer_duration_ms * self.buffer_config.buffer_count as f32;
