...
use std::path::Path;
use voirs::{config::AppConfig, error::Result};
use crate::config::profiles::{ProfileManager, ConfigProfile};
...
pub async fn run_config(
    ...
    if show {
        ...
        let json = serde_json::to_string_pretty(config)
            .map_err(|e| voirs::VoirsError::config_error(format!("Failed to serialize config: {}", e)))?;
        println!("{}", json);
    } else if init {
...
pub async fn run_list_profiles() -> Result<()> {
    ...
    for profile in profiles {
        let current_indicator = if current_profile == Some(&profile.name) { " (current)" } else { "" };
        let system_indicator = if profile.system { " [system]" } else { "" };
        
        println!("  {}{}{}", profile.name, current_indicator, system_indicator);
...
pub async fn run_profile_info(name: &str) -> Result<()> {
    ...
    println!("Created: {}", profile.info.created_at.format("%Y-%m-%d %H:%M:%S UTC"));
    println!("Modified: {}", profile.info.modified_at.format("%Y-%m-%d %H:%M:%S UTC"));
    ...
    println!("--------------");
    println!("Default format: {}", profile.config.cli.default_output_format);
...
pub async fn run_create_profile(name: &str, description: Option<String>, copy_from: Option<String>) -> Result<()> {
    let mut profile_manager = ProfileManager::new()
        .map_err(|e| voirs::VoirsError::config_error(e.to_string()))?;
    
    if let Some(source) = copy_from {
        // Copy from existing profile
        profile_manager.copy_profile(&source, name, description)
            .map_err(|e| voirs::VoirsError::config_error(e.to_string()))?;
        ...
        profile_manager.create_profile(name, description, config)
            .map_err(|e| voirs::VoirsError::config_error(e.to_string()))?;
...
pub async fn run_export_profile(name: &str, path: &Path) -> Result<()> {
    ...
    profile_manager.export_profile(name, path)
        .map_err(|e| voirs::VoirsError::config_error(e.to_string()))?;
...
pub async fn run_import_profile(path: &Path, name: Option<&str>) -> Result<()> {
    ...
    profile_manager.import_profile(path, name)
        .map_err(|e| voirs::VoirsError::config_error(e.to_string()))?;
    
    let imported_name = name.unwrap_or("(from file)");
    println!("Imported profile '{}' from: {}", imported_name, path.display());
