...
use super::{InteractiveOptions, synthesis::SynthesisEngine, session::SessionManager, commands::CommandProcessor};
...
impl InteractiveShell {
    ...
    pub async fn new(options: InteractiveOptions) -> Result<Self> {
        ...
        // Set initial voice
        let current_voice = options.voice.clone()
    ...
    async fn process_command(&mut self, command: &str) -> Result<()> {
        ...
        self.session_manager.add_synthesis(command, &self.current_voice);
    ...
    async fn handle_shell_command(&mut self, command: &str) -> Result<()> {
        self.command_processor.process_command(self, command).await
    }
    ...
    fn create_prompt(&self) -> String {
        ...
        let params = if self.current_speed != 1.0 || self.current_pitch != 0.0 || self.current_volume != 1.0 {
            format!(" [s:{:.1} p:{:.1} v:{:.1}]", self.current_speed, self.current_pitch, self.current_volume)
        } else {
            String::new()
        };
    ...
    fn complete_input(&self, input: &str) -> Vec<String> {
        ...
        if input.starts_with(':') {
            ...
            let commands = [
                ...
                ":save", ":load", ":history", ":clear", ":status", ":quit", ":exit"
            ];
    ...
    fn print_help_hint(&self) {
        println!("{}", style("Hint: Type ':help' for available commands").dim());
        println!();
    }
    ...
    pub async fn set_params(&mut self, speed: Option<f32>, pitch: Option<f32>, volume: Option<f32>) -> Result<()> {
        if let Some(s) = speed {
        ...
        if let Some(v) = volume {
            ...
            self.synthesis_engine.set_volume(self.current_volume).await?;
        }
