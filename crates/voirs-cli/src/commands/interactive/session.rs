...
impl SessionManager {
    ...
    pub async fn load_session<P: AsRef<Path>>(&mut self, path: P) -> Result<()> {
        ...
        let content = tokio::fs::read_to_string(path).await.map_err(|e| {
            VoirsCliError::IoError(format!("Failed to read session file '{}': {}", path.display(), e))
        })?;
        ...
        println!("  Created: {}", self.session_data.metadata.created_at.format("%Y-%m-%d %H:%M:%S UTC"));
    ...
    pub async fn save_session<P: AsRef<Path>>(&mut self, path: P) -> Result<()> {
        ...
        if let Some(parent) = path.parent() {
            tokio::fs::create_dir_all(parent).await.map_err(|e| {
                VoirsCliError::IoError(format!("Failed to create directory '{}': {}", parent.display(), e))
            })?;
        }
        
        tokio::fs::write(path, content).await.map_err(|e| {
            VoirsCliError::IoError(format!("Failed to write session file '{}': {}", path.display(), e))
        })?;
    ...
    pub fn update_voice_settings(&mut self, speed: Option<f32>, pitch: Option<f32>, volume: Option<f32>) {
        if let Some(s) = speed {
    ...
    pub async fn export_history<P: AsRef<Path>>(&self, path: P, format: ExportFormat) -> Result<()> {
        let path = path.as_ref();
