...
impl AudioEncoder {
    ...
    fn write_wav<P: AsRef<Path>>(&self, audio: &AudioBuffer, path: P) -> Result<()> {
        ...
        match self.config.bits_per_sample {
            16 => {
                for &sample in audio.samples() {
                    ...
                    writer.write_sample(sample_i16)
                        .map_err(|e| VocoderError::InputError(format!("Failed to write sample: {}", e)))?;
                }
            }
            24 => {
                for &sample in audio.samples() {
            ...
            _ => return Err(VocoderError::ConfigError("Unsupported bits per sample".to_string())),
        }
...
impl AudioDecoder {
    ...
    pub fn read_wav<P: AsRef<Path>>(path: P) -> Result<AudioBuffer> {
            ...
            SampleFormat::Float => {
                for sample_result in reader.samples::<f32>() {
                    let sample = sample_result
                        .map_err(|e| VocoderError::InputError(format!("Failed to read sample: {}", e)))?;
                    samples.push(sample);
                }
            }
            SampleFormat::Int => {
        ...
        Ok(AudioBuffer::new(samples, spec.sample_rate, spec.channels as u32))
    }
}
...
impl StreamingAudioWriter {
    ...
    pub fn new<P: AsRef<Path>>(path: P, config: AudioEncodeConfig) -> Result<Self> {
        ...
        let writer = WavWriter::create(path, spec)
            .map_err(|e| VocoderError::InputError(format!("Failed to create streaming writer: {}", e)))?;
    ...
    pub fn write_chunk(&mut self, audio: &AudioBuffer) -> Result<()> {
        match self.config.bits_per_sample {
            ...
            24 => {
                for &sample in audio.samples() {
                    let sample_i32 = (sample * 8388607.0).clamp(-8388608.0, 8388607.0) as i32;
                    self.writer.write_sample(sample_i32)
                        .map_err(|e| VocoderError::InputError(format!("Failed to write sample: {}", e)))?;
                }
            }
            32 => {
