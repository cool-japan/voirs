...
pub fn stereo_to_mono(audio: &AudioBuffer) -> Result<AudioBuffer> {
    let samples = audio.samples();
    if samples.len() % 2 != 0 {
        return Err(VocoderError::InputError("Invalid stereo audio length".to_string()));
    }
...
pub fn mix_audio_buffers(buffers: &[AudioBuffer]) -> Result<AudioBuffer> {
    if buffers.is_empty() {
        return Err(VocoderError::InputError("No audio buffers to mix".to_string()));
    }
...
pub fn trim_silence(audio: &AudioBuffer, threshold: f32) -> Result<AudioBuffer> {
    ...
    let start = samples.iter()
        .position(|&x| x.abs() > threshold)
        .unwrap_or(0);
    
    // Find last non-silent sample
    let end = samples.iter()
...
pub fn concatenate_audio(buffers: &[AudioBuffer]) -> Result<AudioBuffer> {
    if buffers.is_empty() {
        return Err(VocoderError::InputError("No audio buffers to concatenate".to_string()));
    }
...
mod tests {
    ...
    fn test_stereo_to_mono() {
        ...
        let expected = vec![0.15, 0.35]; // (0.1+0.2)/2, (0.3+0.4)/2
