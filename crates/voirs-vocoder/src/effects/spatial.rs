...
impl DelayLine {
    ...
    pub fn process(&mut self, input: f32, delay_samples: usize) -> f32 {
        let read_index = (self.index + self.buffer.len() - delay_samples.min(self.buffer.len())) % self.buffer.len();
...
impl StereoWidth {
    ...
    fn update_filters(&mut self) {
        ...
        self.highpass_left.design_highpass(sample_rate, crossover_freq, 0.7);
        self.highpass_right.design_highpass(sample_rate, crossover_freq, 0.7);
    }
