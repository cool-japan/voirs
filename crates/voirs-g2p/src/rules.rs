...
impl G2pRule {
    ...
    pub fn new(pattern: &str, replacement: &str, priority: i32) -> Result<Self> {
        let regex = Regex::new(pattern)
            .map_err(|e| G2pError::ConfigError(format!("Invalid regex pattern '{}': {}", pattern, e)))?;
    ...
    pub fn with_context(pattern: &str, replacement: &str, priority: i32, context: &str) -> Result<Self> {
...
impl EnglishRuleG2p {
    ...
    fn create_phoneme_with_features(symbol: &str, ipa: &str, features: PhoneticFeatures) -> Phoneme {
        Phoneme::full(
            symbol.to_string(),
            Some(ipa.to_string()),
    ...
    fn create_vowel(symbol: &str, ipa: &str, height: &str, frontness: &str, rounded: bool) -> Phoneme {
    ...
    fn create_consonant(symbol: &str, ipa: &str, manner: &str, place: &str, voiced: bool) -> Phoneme {
        ...
        for (i, word) in words.iter().enumerate() {
            ...
            if let Some(dict_phonemes) = self.word_dict.get(&clean_word) {
                for phoneme_str in dict_phonemes {
                    // Create phoneme with IPA symbol (same as symbol for dictionary entries)
                    all_phonemes.push(Phoneme::with_ipa(phoneme_str, phoneme_str));
                }
            } else {
                // Apply rules
...
impl G2p for EnglishRuleG2p {
    async fn to_phonemes(&self, text: &str, _lang: Option<LanguageCode>) -> Result<Vec<Phoneme>> {
        tracing::debug!("EnglishRuleG2p: Converting '{}' to phonemes", text);
        ...
        tracing::debug!("EnglishRuleG2p: Generated {} phonemes", phonemes.len());
        Ok(phonemes)
    }
