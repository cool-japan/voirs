...
use voirs_g2p::{DummyG2p, G2p, LanguageCode, Phoneme, Result};
...
async fn test_basic_word_conversion() -> Result<()> {
    ...
    for (word, expected_phonemes) in test_cases {
        ...
        println!("Word: '{}' -> {:?} (expected: {:?})", word, phoneme_symbols, expected_phonemes);
        
        // For rule-based system, we may have variations, so check if output is reasonable
        assert!(!phonemes.is_empty(), "Should generate phonemes for '{}'", word);
        assert!(phonemes.len() >= 2, "Should generate multiple phonemes for '{}'", word);
    }
...
async fn test_magic_e_patterns() -> Result<()> {
    ...
    let magic_e_words = vec![
        "cake", "make", "take", "lake", "name", "game", "same",
        "bite", "kite", "site", "cute", "mute", "tube", "hope", "rope",
    ];
    
    for word in magic_e_words {
        let phonemes = g2p.to_phonemes(word, Some(LanguageCode::EnUs)).await?;
        println!("Magic-e word '{}' -> {:?}", word, 
                phonemes.iter().map(|p| &p.symbol).collect::<Vec<_>>());
        
        assert!(!phonemes.is_empty(), "Should generate phonemes for magic-e word '{}'", word);
        // Magic-e words typically have long vowel sounds
        assert!(phonemes.len() >= 2, "Magic-e words should have multiple phonemes");
    }
...
async fn test_consonant_clusters() -> Result<()> {
    ...
    let cluster_words = vec![
        ("street", "str"), ("spring", "spr"), ("splash", "spl"),
        ("throw", "thr"), ("school", "sch"), ("chrome", "chr"),
        ("blend", "bl"), ("grape", "gr"), ("clock", "cl"),
    ];
    
    for (word, cluster) in cluster_words {
        let phonemes = g2p.to_phonemes(word, Some(LanguageCode::EnUs)).await?;
        println!("Cluster word '{}' (cluster: {}) -> {:?}", word, cluster,
                phonemes.iter().map(|p| &p.symbol).collect::<Vec<_>>());
        
        assert!(!phonemes.is_empty(), "Should generate phonemes for cluster word '{}'", word);
        assert!(phonemes.len() >= 3, "Cluster words should have multiple phonemes");
    }
...
async fn test_sentence_processing() -> Result<()> {
    ...
    for sentence in sentences {
        ...
        assert!(!phonemes.is_empty(), "Should generate phonemes for sentence");
        assert!(phonemes.len() >= sentence.split_whitespace().count() * 2, 
        ...
        if sentence.contains(' ') {
            assert!(has_spaces, "Multi-word sentences should contain space phonemes");
        }
    }
...
async fn test_edge_cases() -> Result<()> {
    ...
    assert!(phonemes.is_empty(), "Empty input should produce no phonemes");
    
    // Test single character
    let phonemes = g2p.to_phonemes("a", Some(LanguageCode::EnUs)).await?;
    assert!(!phonemes.is_empty(), "Single character should produce phonemes");
    
    // Test punctuation
    let phonemes = g2p.to_phonemes("Hello!", Some(LanguageCode::EnUs)).await?;
    assert!(!phonemes.is_empty(), "Text with punctuation should produce phonemes");
    
    // Test numbers (if implemented)
    let phonemes = g2p.to_phonemes("test 123", Some(LanguageCode::EnUs)).await?;
    println!("Numbers test: {:?}", phonemes.iter().map(|p| &p.symbol).collect::<Vec<_>>());
    
    // Test mixed case
    let phonemes = g2p.to_phonemes("Hello World", Some(LanguageCode::EnUs)).await?;
...
async fn test_vowel_patterns() -> Result<()> {
    ...
    let vowel_tests = vec![
        // Long vowels
        ("beat", "IY"), ("boat", "OW"), ("boot", "UW"),
        // Short vowels  
        ("bit", "IH"), ("bet", "EH"), ("bat", "AE"),
        // Diphthongs
        ("boy", "OY"), ("cow", "AW"), ("buy", "AY"),
        // R-controlled
        ("car", "AR"), ("her", "ER"), ("for", "OR"),
    ];
    ...
    for (word, vowel_sound) in vowel_tests {
        ...
        println!("Vowel test '{}' -> {:?} (should contain sound like {})", 
                word, phoneme_symbols, vowel_sound);
        
        assert!(!phonemes.is_empty(), "Should generate phonemes for vowel test word '{}'", word);
    }
...
async fn test_irregular_words() -> Result<()> {
    ...
    let irregular_words = vec![
        "one", "two", "eight", "through", "though", "rough",
        "said", "says", "women", "colonel", "island"
    ];
    
    for word in irregular_words {
        let phonemes = g2p.to_phonemes(word, Some(LanguageCode::EnUs)).await?;
        println!("Irregular word '{}' -> {:?}", word,
                phonemes.iter().map(|p| &p.symbol).collect::<Vec<_>>());
        
        assert!(!phonemes.is_empty(), "Should generate phonemes for irregular word '{}'", word);
    }
...
async fn test_stress_patterns() -> Result<()> {
    ...
    let stress_words = vec![
        "computer", "telephone", "information", "beautiful",
        "interesting", "development", "understanding"
    ];
    
    for word in stress_words {
        let phonemes = g2p.to_phonemes(word, Some(LanguageCode::EnUs)).await?;
        println!("Stress word '{}' -> {:?}", word,
                phonemes.iter().map(|p| &p.symbol).collect::<Vec<_>>());
        
        assert!(!phonemes.is_empty(), "Should generate phonemes for stress word '{}'", word);
        assert!(phonemes.len() >= 4, "Multi-syllable words should have multiple phonemes");
    }
...
async fn test_performance_benchmark() -> Result<()> {
    ...
    assert!(!phonemes.is_empty(), "Should generate phonemes for performance test");
    assert!(duration.as_millis() < 100, "Should process text quickly (< 100ms)");
...
async fn test_configuration_options() -> Result<()> {
    ...
    let phonemes = g2p.to_phonemes("Hello, world!", Some(LanguageCode::EnUs)).await?;
    println!("Configured G2P output: {:?}", 
            phonemes.iter().map(|p| &p.symbol).collect::<Vec<_>>());
    
    assert!(!phonemes.is_empty(), "Should generate phonemes with custom config");
